...
const ServerInfoPage = ({ servers }: ServerInfoState) => {
    const { id } = useParams();
    const defaultServer: Server = { config: "", id: "", password: "", serverCountry: "", serverIp: "", serverName: "", username: "" }
    const [loading, setLoading] = useState(false);
    const [showDeleteDialog, setShowDeleteDialog] = useState(false);
    const [server, setServer] = useState(defaultServer);
    const serversData = servers.servers
    console.log("SERVER INFO", serversData);
    ...
    const uploadServer = (server: Server, isNew: boolean) => {
        ...
        console.log("Upload server: ", server, isNew)
        return success
    }
